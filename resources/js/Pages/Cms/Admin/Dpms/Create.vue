      ...
      <form @submit.prevent="submit">
          <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
            <div class="sm:col-span-2">
                  <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">รหัสสาขา</label>
                  <input type="text" name="name" id="name"
                  v-model="form.code"
                  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="รหัสสาขา" required="">
              </div>
              <div class="sm:col-span-2">
                  <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">ชื่อฝ่าย</label>
                  <input type="text" name="name" id="name"
                  v-model="form.name"
...
export default {
  ...
  methods: {
    submit() {
      ...
      this.$inertia.post(`/cms/managedpms`, this.form, {
        onSuccess: () => {
         console.log('ok');
        },
        onError: () => {
